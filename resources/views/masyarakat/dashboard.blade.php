...
<div class="container">
    <div class="card shadow-sm mb-4" data-aos="fade-down">
        ...
        <div class="card-body">
            <p class="lead">Halo, <strong>{{ auth()->user()->name }}</strong>! Anda login sebagai <strong>{{ auth()->user()->role }}</strong>.</p>
            <p>Silakan ajukan proposal bantuan melalui sistem ini.</p>

            <a href="{{ route('proposals.create') }}" class="btn btn-success btn-lg mt-3 animate__animated animate__pulse animate__infinite">Ajukan Proposal Sekarang</a>
        </div>
    </div>
