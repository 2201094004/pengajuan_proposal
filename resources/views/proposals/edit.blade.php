            ...
            <form action="{{ route('proposals.update', $proposal->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="kabupaten_tujuan" name="kabupaten_tujuan" required>
                        ...
                        @foreach($kabupatens as $kabupaten)
                            <option value="{{ $kabupaten->nama }}" {{ $proposal->kabupaten_tujuan == $kabupaten->nama ? 'selected' : '' }}>{{ $kabupaten->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    @if($proposal->document)
                        <small class="form-text text-muted">Dokumen saat ini: 
                            <a href="{{ asset('storage/documents/' . $proposal->document) }}" target="_blank">Lihat</a>
                        </small>
                    @endif
                </div>
